...
client.on("guildMemberAdd", (guildMember) => {
  let welcomeRole = guildMember.guild.roles.cache.find(
    (role) => role.name === "swag pending"
  );

  guildMember.roles.add(welcomeRole);
  guildMember.guild.channels.cache
    .get("824491352153653260")
    .send(
      `Welcome <@${guildMember.user.id}> to swagalicious homies! Please read the <#824498701657440296> and do -acceptrules in <#826667824449323008>!`
    );
...
function sendMessage() {
  ...
  const newEmbed = new Discord.MessageEmbed({
    ...
    .setThumbnail(selectedQuote.thumbnail);
